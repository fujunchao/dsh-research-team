...
**成稿质量门（`assessDraft`）**：成员产出在落档/送审/修订回写前统一校验——长度下限（章节 800 字符、修订按原稿 0.6 折算）、占位符骨架（`[6 prose paragraphs, ~1400字]` 等）、英文过程自述（`JACKPOT`/`Word count check` 等）、中文占比 <30%、缺【本章小结】/「本章新增来源」标记、悬空标题或"以下为全文"式引导句收尾。不过门 → 附理由重派 1 次（硬性要求前置到任务最前，并要求"已写文件的成稿直接内联回传"）；仍不过 → **工作区成稿抢救**（扫描派发期间修改的最新 `.md`，内容过门则采纳——成员把成稿写进文件、最终输出只留元话语是 glm-5.3-flash 的实证常态）；抢救也失败才走降级表。续跑时对旧断点遗留的不合格落档草稿同样拦截（清除后重研，不再送审垃圾稿）。初调摘要在任务卡中封顶 6000 字符（超长摘要会淹没章节任务指令、放大输出失稳）。成员超时/重派/降级事件随参数卡写入 checkpoint `card.log`（上限 200 行），排查不再依赖子代理 transcript；降级占位的章数通过 `degradedChapters` 字段透出并在工具结果中显示警告。
