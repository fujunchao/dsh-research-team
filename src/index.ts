...
import { runResearch, reviseChapter, type RunTracker } from './orchestrator.js'
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          status: { type: 'string', required: true, description: 'completed | awaiting-outline-confirm | error' },
      ...
      render(args, value) {
        const v = value as { ok?: boolean; status?: string; planId?: string; title?: string; reportPath?: string; error?: string; sourceCount?: number; outline?: string[] }
        const text = !v?.ok
          ? `❌ 深度研究失败：${v?.error ?? '未知错误'}`
    ...
    async execute(args, exec) {
      ...
      if (args.planId !== undefined) {
        const entry = plans.getPlan(String(args.planId))
        if (!entry) {
          return { ok: false, status: 'error', error: `planId ${args.planId} 不存在（可能已过期或 dsh 已重启，最多保留 10 个）`, planId: '', reportPath: '', chapters: [], sourceCount: 0, reportExcerpt: '', htmlPath: '', title: '', outline: [], message: '', progressLog: progress }
        }
        if (entry.stage === 'executing') {
          return { ok: false, status: 'error', error: `planId ${args.planId} 正在执行中，请等待完成后再调用`, planId: '', reportPath: '', chapters: [], sourceCount: 0, reportExcerpt: '', htmlPath: '', title: '', outline: [], message: '', progressLog: progress }
        }
        ...
        try {
          ...
          if (args.reviseChapter !== undefined) {
            ...
            const result = {
              ok: true, status: 'completed', planId: entry.planId,
          ...
          if (args.outlineFeedback) {
            ...
            const stop = await runResearch(ctx, card, exec.agent, exec.signal, progress_, track, String(args.outlineFeedback), true)
            ...
            const stop = await runResearch(ctx, card, exec.agent, exec.signal, progress_, track, undefined, true)
            if (stop === 'awaiting-outline-confirm') {
      ...
      try {
        ...
        const stop = await runResearch(ctx, card, exec.agent, exec.signal, progress_, track, undefined, undefined, skipConfirm)
        if (stop === 'awaiting-outline-confirm') {
