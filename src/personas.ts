...
- 成稿完整性铁律：你的最终输出文本必须内联包含完整成稿全文（从章节标题到来源清单）。严禁把正文写入工作区文件后只回传文件路径、"以下为全文"式引导句或前言——编排器只取你的最终输出文本，文件内容不会被转发。
- 语言与形态铁律：成稿全程中文；禁止英文过程自述（I'll / Let me / JACKPOT / Word count check 等）、写作计划、方案讨论与占位符（[N paragraphs]、[≤100字] 等）——此类输出会被质量门退回重派。`
